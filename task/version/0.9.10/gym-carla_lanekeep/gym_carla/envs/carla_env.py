...
from gym_carla.envs.global_route_planner_dao import GlobalRoutePlannerDAO
import numpy as np
import pygame
import random
import time
...
from gym_carla.envs.local_planner import LocalPlanner
...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, params):
        ...
        # 获得车辆行驶的轨迹长度，然后做奖励
        self.r_trajectory_now = 0
        self.r_trajectory_last = 0
    ...
    def reset(self):
      ...
      # start 
        # Spawn surrounding vehicle随机放置环境车辆
        ...
        self.localplanner = RoutePlanner(self.ego, self.max_waypt)
    ...
    def _try_spawn_control_vehicle_at(self, transform, throttle, steer, brake, hand_brake, number_of_wheels=[4]):
        ...
        blueprint = self._create_vehicle_bluepprint(
            'vehicle.tesla.model*', number_of_wheels=number_of_wheels)
    ...
    def _get_obs(self):
        ...
        for location in self.lidar_data:
            ...
            # print(location.__dir__())
            # print('--------------------', location.point.x,  type( location.point))
        ...
        # 状态包括：车辆与车道线的横向距离，与车道线的夹角，当前速度，和前方车辆的距离
        state = np.array([lateral_dis, - delta_yaw, speed, self.vehicle_front])
        # print('vehicle location:', ego_x, ego_y)
        ...
        if self.pixor:
            ...
            # print('vh_clas:',vh_clas.shape) # (64,64)
            # print('vh_regr:',vh_regr.shape) # (64,64,6)

            # Pixor state, [x, y, cos(yaw), sin(yaw), speed]
            pixor_state = [ego_x, ego_y, np.cos(
                ego_yaw), np.sin(ego_yaw), speed]
            # print('pixor_state:',pixor_state)
    ...
    def _get_reward(self):
        ...
        if self.vehicle_front:
            r_brake = self.ego.get_control().brake
            # print('r_brake:',r_brake)
        ...
        # 如果要超出车道线，进行加速就惩罚
        r_throttle = self.ego.get_control().throttle
        if abs(dis) >= 1.5:
            r_throttle = -5*self.ego.get_control().throttle
            lspeed_lon = 0.1*lspeed_lon
        # print('r_throttle:',r_throttle)
        ...
        if lspeed_lon > self.desired_speed:
            r_fast = -1
            r_throttle = -5*self.ego.get_control().throttle
            r_brake = 5*self.ego.get_control().brake

        # cost for lateral acceleration 横向加速度
    ...
    def _trace_route(self, start_waypoint, end_waypoint):
        ...
        if self._grp is None:
            dao = GlobalRoutePlannerDAO(
                self.ego.get_world().get_map(), self._hop_resolution)
